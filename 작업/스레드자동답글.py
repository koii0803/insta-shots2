...
다시오기막는날 = 7               # 이 날짜 안에 이미 봐준 사람은 **조용히 무시** (2026-09-25 사장님 지시)
MAX_PER_PERSON = 5              # 한 글에서 같은 사람과 최대 몇 번 (2026-09-25 사장님 지시 10 → 5).
                                # 5턴에 유료 안내하고 끊는다. 최근 3일 206건 중 6턴 넘는 건 9건(4.4%)뿐
PROMO_TURN = 1                  # 스크랩·하트·리포스트를 청하는 턴 (2026-09-25 사장님 지시 3 → 1).
                                # 1턴이 전체의 40.8%%(84/206)다. 3턴부터면 그 84명이 아예 못 듣고 떠났다
PROFILE_TURN = 3                # 디엠 안내는 **3턴 고정** (2026-09-25 사장님 지시).
                                # 2턴 46명 / 3턴 35명 / 4턴 18명 — 3턴이 붙은 사람에 닿고 아직 안 빠진 자리
...
# ── 문지기 (2026-09-27 사장님) ─────────────────────────────────────────
# 무시는 **딱 두 가지**다.  ① 일반 인사(인사·감사·칭찬·웃음·감탄)  ② 스팸(링크·영업·도배)
# 나머지는 짧아도 전부 AI 분류기로 간다. 벽은 좁게. (서아 타로봇/분류.py 와 같은 생각, 코드는 따로)
# 떠보기 규칙은 **팔자오빠엔 안 둔다** (2026-09-27 사장님: "팔자오빠는 이제 떠보는 사람 안 온다").
#   괜히 두면 정상 손님만 잘못 걸린다. 혹시 와도 AI 분류기의 '함정' 판단이 뒤에 있다
# 팔자오빠는 번호 글이 없고 **숫자만 쓴 댓글이 생년월일**일 수 있어서 숫자는 안 막는다.
# 1층 인사: **인사말로만 이뤄진 댓글**만. 다른 말이 섞이면 1층은 손 안 대고 3층(분류기 light)이 본다
#   (군말 목록으로 걷어 내던 방식은 목록이 길수록 오판이 늘어서 뺐다)
인사말 = sorted({
    "감사합니다", "감사해요", "감사드려요", "감사드립니다", "감사", "정말감사합니다", "너무감사합니다", "진짜감사합니다",
    "고맙습니다", "고마워요", "고마워", "땡큐", "땡스", "thankyou", "thanks", "thx",
    "화이팅", "파이팅", "홧팅", "응원합니다", "응원해요", "반가워요", "반갑습니다", "안녕하세요", "안녕",
    "잘봤어요", "잘봤습니다", "잘봤어", "잘보고갑니다", "잘보고가요", "좋아요", "좋네요", "최고", "최고예요", "최고에요",
    "대박", "소름", "신기해요", "신기하다", "짱", "굿", "굿굿", "good", "nice", "wow", "우와", "와우", "오오", "헐",
    "굿밤", "좋은밤", "편안한밤되세요", "좋은하루되세요", "행복하세요", "수고하세요", "용하다", "용하네요", "용하시네요",
}, key=len, reverse=True)
감탄한마디 = {"오", "와", "오오", "우와", "와우", "헐", "굿", "굿굿", "짱", "캬", "크", "흠", "아", "앗", "오호"}
대답말 = {"네", "넵", "넹", "예", "응", "웅", "ㅇㅇ", "ㅇㅋ", "ok", "okay", "오케이", "알겠어요", "알겠습니다"}
영업말 = ("맞팔", "선팔", "소통해", "소통하실", "품앗이", "반하리", "맞스하리", "1000명프로젝트", "천명프로젝트", "협찬", "홍보")
# "가요" 는 안 넣는다 — "선팔하고 가요" 가 질문으로 잡힌다 ("인가요" 는 따로 있다)
물음꼴 = re.compile(r"(까요|까\?|을까|ㄹ까|나요|어때|어떨|언제|누가|뭐야|뭔가|뭘까|인가요|\?|궁금|봐주|봐줘|알려"
                    r"|어떻게|어떡|될지|할지|있을지|올지|싶어|고민|해야|모르겠)")


def _알맹이(t):
    return re.sub(r"[^0-9A-Za-z가-힣ㄱ-ㅎㅏ-ㅣ]", "", t or "").lower()


def 문지기(t, used=0):
    """무시할 댓글이면 까닭, 아니면 None. **무시는 일반 인사·악성 둘뿐.**"""
    낮 = (t or "").lower()
    # ② 스팸
    if re.search(r"https?://|www\.|t\.me/|오픈채팅|open\.kakao", 낮):
        return "악성: 링크·홍보"
    걸린 = next((w for w in 영업말 if w in 낮.replace(" ", "")), "")
    if 걸린 and not 물음꼴.search(t) and not re.search(r"\d{2}", t):
        return "악성: 영업(%s)" % 걸린
    a = _알맹이(t)
    if len(a) >= 6 and len(set(a)) <= 2 and not a.isdigit():
        return "악성: 도배"
    # ① 일반 인사
    if not a:
        return "일반 인사: 이모지·기호뿐"
    if used >= 1 and a in 대답말:
        return None                            # 내가 물은 데 "넵" — 대답이다 (분류기가 본다)
    if re.fullmatch(r"[ㄱ-ㅎㅏ-ㅣ]+", a):
        return "일반 인사: 자음·모음뿐"
    if re.fullmatch(r"[하히호흐크키푸헤]{2,}", a):
        return "일반 인사: 웃음뿐"
    if a in 감탄한마디:
        return "일반 인사: 감탄 한마디"
    if used == 0 and a in 대답말:
        return "일반 인사: 물은 적 없는데 대답만"
    남 = re.sub(r"[ㄱ-ㅎㅏ-ㅣ]", "", a)
    for w in 인사말:
        남 = 남.replace(w, "")
    if not 남:
...
def prefilter(row):
    ...
    try:
        ...
        pass
    # 7일 안에 이미 봐준 사람은 조용히 무시한다 (2026-09-25).
    # **이 글에서 이미 주고받는 중이면 빼고** — 그건 이어지는 대화지 다시 온 게 아니다
    try:
        if STORE is not None and row.get("used", 0) == 0:
            언제 = STORE.푼사람_읽기().get(row["reply"].get("username") or "")
            if 언제:
                지난날 = (now().date() - dt.datetime.strptime(언제, "%Y-%m-%d").date()).days
                if 0 <= 지난날 < 다시오기막는날:
    ...
    if not t:
        return "빈 글"
    # **문지기** (2026-09-27 사장님): 무시는 딱 두 가지 — 일반 인사 · 악성. 나머지는 짧아도 전부 답한다.
    #   전엔 "3글자 이하는 무시" 라서 봇이 "남자야 여자야?" 물어 놓고 "여자" 를 버렸다. 시험지 문지기시험.py
    why = 문지기(t, row.get("used", 0))
    if why:
        return why
...
# ── 파이썬이 짧다고 버린 댓글 → 앞단(내 글 + 오간 말)과 같이 AI 가 한 번 가른다 ──────
# 2026-09-27 사장님: "파이썬이 무시한 댓글은 앞단을 같이 가져와서 AI 한테 토스해".
#   전엔 이 판단을 AI자동답변6시간.py 가 **6시간마다** 했다. 봇이 "남자야 여자야?" 물어 놓고
#   "여자" 라는 대답을 몇 시간씩 세워 뒀다. 이제 본체가 그 자리(5분 바퀴)에서 한다.
#   AI자동답변 은 보류목록(분류기가 못 정한 것)만 6시간마다 본다. 판단은 여기 하나를 같이 쓴다.
# ⚠️ 칸 이름은 **영문만 된다** (한글이면 서버가 통째로 거절, 2026-09-24 실측)
짧은판단_SCHEMA = {
    "type": "object",
    "properties": {
...
짧은판단_SYS = "너는 대화가 이어지는 중인지 끝났는지만 가른다. 지시한 JSON 하나만 출력한다."
짧은판단확신 = 70          # 이 아래면 안 단다. 애매하면 조용한 쪽이 낫다
짧은판단_틀 = """스레드 계정 @paljaoppa(팔자오빠) 사주 상담 대화다.
마지막 댓글을 기계가 판단 못 했다(너무 짧거나 애매하다). 내 글과 앞 대화를 보고 갈라라.
...
**이어짐** — 대화를 이어 가야 한다
  ...
  · 내 글이 물은 것에 대한 대답이다 (예: 글이 "요즘 뭐가 제일 걱정이야?" -> "돈")
...
def 짧은댓글판단(row, 부르는곳="짧은댓글"):
    """(판단, 확신, 까닭). 판단은 '이어짐'/'끝남'. 오류면 ('끝남', 0, 이유) — 막히면 안 다는 쪽으로."""
    누구 = row["reply"].get("username") or "?"
    # 줄기(chain) 맨 끝은 방금 단 그 댓글이다. 그대로 넣으면 [방금 달린 댓글] 과 두 번 나와서
    #   AI 가 "같은 말을 되풀이했다" 며 끝남으로 본다 (2026-09-27 시험: "돈" 이 그렇게 떨어졌다)
    앞 = list(row.get("chain") or [])
    if 앞 and 앞[-1].startswith("@%s:" % 누구):
        앞 = 앞[:-1]
    out = claude(
        짧은판단_틀 % ((row["post"].get("text") or "(모름)").strip()[:600],
                    chr(10).join(앞) or "(없음)",
                    누구,
                    (row["reply"].get("text") or "").strip()),
        짧은판단_SCHEMA, MODEL_EXTRACT, 짧은판단_SYS, timeout=120, 부르는곳=부르는곳)
    if "__오류__" in out:
...
- 보류 : **여기는 "답할지 말지 내가 못 정하겠다" 는 자리다.** 버리는 자리가 아니다.
  ...
  **★ 감사·인사·가벼운 반응은 label 은 '답함' 으로 두고 light = true** 로 표시해라.
    "감사합니다" "고마워" "응 스하리~" "편안한 밤 되세요" 같은 말이다.
    (2026-09-27 사장님: **일반 인사는 무시.** light 가 true 면 답이 안 나간다)
...
**1) 사주 사실은 엔진 값에만 근거한다.** 위 표에 없는 글자·관계·연도를 지어내면 안 된다. 상대가 검증하면 바로 들킨다.
   **다만 사람 얘기는 표 밖에서 해도 된다** (2026-09-25 사장님 지시).
   짚어 주는 것은 사주로 짚을 때도 있고 **사람 마음으로 짚을 때도 있어야 한다.**
   · 표에서만 나오는 것 — 일주·십신·오행 개수·충·합·원진·대운·세운·연도. **하나도 지어내지 마라.**
   · 표 없이 해도 되는 것 — 이 사람이 지금 무슨 마음일지, 왜 그랬을지, 뭘 못 말하고 있는지.
     사람 사는 얘기라 글자로 증명할 게 아니다.
   · 사람 얘기는 **단정하지 말고 "~했을 거야" · "그런 거 아니야?" 결로** 둬라. 틀리면 바로 수긍하면 된다.
   · 한 글에 글자 얘기만 가득 차면 안 된다. 손님은 사람 얘기를 하러 왔다.
...
TURN_GUIDE = {
    ...
    "보통": ("**끝을 어떻게 맺을지 네가 정해라** (2026-09-25 사장님 지시).\n"
             "   · 이 사람이 쏟아내고 싶어 보이면 **물어서 더 나오게 해라.** 위로로 닫으면 감사합니다 로 끝난다\n"
             "   · 할 말을 다 했으면 **안 물어도 된다.** 억지 질문은 취조가 된다\n"
             "   · 묻기로 했으면 **이미 적어 준 걸 다시 묻지 마라.** 그건 못 읽은 것이다\n"
             "   대화가 여기서 끊기면 채널이 안 크니, 애매하면 묻는 쪽이 낫다."),
    ...
    "리포스트": ("풀이에서 자연스럽게 이어지는 걸 하나 물어라.\n"
                 ...
                 "   ★ 줄 순서: … 풀이 … → (끝에서 둘째 줄) 리포스트 한 줄 → 마지막 줄.\n"
                 "     **리포스트 줄로 글을 끝내지 마라.** 그 뒤에 한 줄이 더 와야 한다.\n"
                 "     그 마지막 줄을 물음으로 할지 받아 주는 말로 할지는 네가 정해라."),
    ...
    "유료": ("풀이에서 자연스럽게 이어지는 걸 하나 물어라.\n"
             ...
             "   ★ 줄 순서: … 풀이 … → (끝에서 둘째 줄) 디엠 한 줄 → 마지막 줄.\n"
             "     디엠 줄로 글을 끝내지 마라. 마지막 줄을 물음으로 할지 받아 주는 말로 할지는 네가 정해라."),
...
# ── 톤은 **AI 가 정한다** (2026-09-25 사장님 지시) ────────────────────
#
# 전에는 턴 숫자로 못 박았다 — 1~2턴 전문가 톤·위로 금지, 3~4턴 풀어주기, 5턴+ 사람 대 사람.
# 그랬더니 1~2턴이 전부 글자 얘기가 됐다. "일지 오화가 상관이라", "축오 원진이 걸려서".
# 손님은 사람 얘기를 하러 왔는데 기계가 표를 읽어 주니 할 말이 "아 네" 밖에 안 남는다.
#
# 사장님 말: "사주 보는 사람은 배설을 해야 한다. 궁금하니까, 답답하니까.
#            배설을 들어줄 수 있는 상대가 필요한 거다."
# 손님 글을 읽어야 아는 것을 턴 숫자로 퉁칠 수 없다. 그래서 AI 에게 맡긴다.
TONE_ALL = """**이 사람이 지금 뭘 원하는지 네가 읽고 정해라.** 턴 숫자는 참고만 해라.
   세 가지 중 하나다. 글을 보고 골라라.

   가. **쏟아내고 싶다** — 하소연, 답답함, 길게 쓴 사연, 같은 말 반복.
       **표를 먼저 읊지 마라.** 글자 얘기로 시작하면 할 말이 "아 네" 밖에 안 남는다.
       받아 주고 **더 묻는 걸로 받아라.** 위로로 받으면 "감사합니다" 로 닫힌다.
       사주는 아무 데나 찌르라고 있는 게 아니라 **아픈 자리를 먼저 찍고 묻는 데** 쓴다.
       (예: 원진이 걸려 있으면 "이유 없이 긁히는 게 있었지?" 하고 묻는다)

   나. **답을 듣고 싶다** — 또렷한 질문 하나, 시기를 묻는 것, 되냐 안 되냐.
       빙빙 돌리지 말고 근거부터 대라. 여기선 글자 얘기가 맞다.

   다. **아직 널 재보고 있다** — 첫 마디가 짧다, 시험하듯 묻는다, 의심한다.
       실력으로 눌러라. 상대가 "어 맞네" 할 만한 걸 먼저 말해라. 위로는 아직 이르다.

   **위로를 언제 할지도 네가 정해라.** 순서표는 없다. 이 사람이 받을 준비가 됐나만 봐라.
   **되물을지 그냥 받아줄지도 네가 정해라.** 매번 묻지 않아도 된다."""


# ── 사람이 왜 갈라서는가 (2026-09-25 사장님 고찰) ─────────────────────
# 그대로 베껴 쓰라고 두는 게 아니다. **봇이 뭘 먼저 보는지**를 바꾸는 틀이다.
# 사장님 말을 엔진이 내주는 관계(충·원진·합)에 붙여 놨으니 지어낸 말이 아니다.
사람틀 = """**사람이 갈라서는 결은 세 가지다. 이 사람이 어느 쪽인지부터 봐라.**
   싫은 마음이 차오르는 속도가 사람마다 다르고, 그게 원국에도 적혀 있다.

   · **빨리 차오르고 빨리 터진다** = 충(沖) 자리. 정면으로 부딪혀 바로 갈라선다.
     대신 돌아올 길이 아주 막히진 않는다.
   · **오래 쌓이다 한 번에 터진다** = 원진(怨嗔) 자리. 이유 없이 긁히는 게 계속 쌓인다.
     터지면 크고, 돌아오기는 훨씬 어렵다.
   · **거의 안 차오른다** = 합(合) 자리. 못 끊고 계속 붙는다.
     대신 시작 자체가 잘 안 걸린다.

   **판정해서 통보하지 마라.** 물어서 이 사람 입으로 나오게 해라.
   (예: "갑자기였어, 아니면 쌓이다 터진 거야?")

   **"내가 이만큼 해줬는데" 라는 말이 나오면** 돌려받을 생각이 있었다는 뜻이다.
   그래도 **"그건 거래였다" 고 못 박지 마라.** 이미 아픈 사람이다.
   물음으로만 둬라 (예: "그거 해줄 때 돌려받을 생각 있었어?").
   답은 이 사람이 낸다. 사람 마음은 아무도 모른다."""


def tone_for(turn):
    """턴에 상관없이 같은 지시. 고르는 건 AI 가 한다 (2026-09-25)."""
    return TONE_ALL + "\n\n   " + 사람틀
...
리포스트말 = ("스하리", "스크랩", "리포스트", "복채", "팔로우", "하트")   # 1턴에 청하는 것
...
PROMO_LINES = [
    "도움 됐으면 스크랩 하트 리포스트 한 번씩만 해주라, 그게 나한텐 복채다",
    "복채는 안 받는다, 대신 스크랩 하트 리포스트 그거면 충분하다",
    "값은 안 받는다, 스크랩 하트 리포스트 손가락 세 번이면 된다",
    "괜찮았으면 스크랩 하트 리포스트 한 번씩, 그거면 나는 됐다",
    "나 돈 받고 하는 거 아니라 스크랩 하트 리포스트가 전부다",
    "읽고 도움 됐으면 스크랩 하트 리포스트만 눌러주라, 그거 하나가 크다",
    "돈 대신 스크랩 하트 리포스트 받는다, 그게 제일 고맙다",
    "맞는 것 같으면 스크랩 하트 리포스트 해주라, 그거 보고 또 누가 온다",
]
...
def handle(row, dry=False):
    ...
    # 일반 인사는 무시 (2026-09-27 사장님). 전엔 짧게 한마디 받았다(가볍게)
    if label == "답함" and ex.get("light"):
        return "인사", conf, ex.get("reason", ""), None, None
    ...
    # **되물을지는 AI 가 정한다** (2026-09-25 사장님 지시).
    # 전에는 마지막 턴 빼고 전부 물음표로 끝나야 통과였다. 그래서 받아 주면 될 자리에도
    # 억지 질문이 붙었고, 따뜻하게 닫은 멀쩡한 글이 세 번 반려되고 버려졌다.
    # 여기서는 **아예 안 막는다.** 언제 묻고 언제 받아 줄지는 시킬 때 말로 일러 둔다.
    ...
    # 클로드가 빈 껍데기("test" · 한글 0자)만 세 번 뱉은 건 **판단이 아니라 기계 딸꾹질**이다 (2026-09-27).
    #   전엔 사장님 텔레그램 확인으로 넘겨서, 생년월일 다 적은 손님(@chunghaheo)이 몇 시간 섰다.
    #   다음 회차에 다시 한다 (계속 안 되면 한 시간에 한 번 '재시도' 알림이 간다)
    if fix.startswith("껍데기"):
        return "보류", conf, "클로드가 빈 답만 냄: " + fix, None, "작성 실패(빈 답)"
...
def run(dry=False):
    ...
    for q in sorted(queue, key=lambda x: x["due"]):           # 오래 기다린 것부터
        ...
        try:
            ...
            try:
                if q.get("user"):
                    STORE.푼사람_적기(q["user"], now().strftime("%Y-%m-%d"))
            except Exception:
                pass
    ...
    for r in todo:
        ...
        # 클로드 빈 답(껍데기)으로 다시 하는 건 **3번까지** (2026-09-27 사장님: 같은 걸 계속 집어 AI 가 보면 토큰 샌다).
        #   계속 빈 답이면 사장님 텔레그램으로 넘긴다. 사용량 한도·시간 초과 같은 건 풀리면 되니 그대로 다시 한다
        빈답수 = state.setdefault("빈답수", {})
        if label == "보류" and reason.startswith("클로드가 빈 답만 냄"):
            빈답수[rid] = 빈답수.get(rid, 0) + 1
            if 빈답수[rid] >= 3:
                label, hold = "보고", "클로드가 %d번 연속 빈 답만 냄 — 사장님이 봐줘" % 빈답수.pop(rid)
        else:
            빈답수.pop(rid, None)
