...
def run(dry=False):
    ...
    for r in todo:
        ...
        why = prefilter(r)
        # 파이썬이 **짧다고만** 버린 것은 앞단(내 글 + 오간 말)을 붙여 AI 에게 한 번 묻는다 (2026-09-27 사장님).
        #   이모지·자음뿐("ㅋㅋ" "ㅇㅇ")은 안 묻는다 — 한글 글자나 숫자가 있어야 말이다
        if why and why.startswith("너무 짧음") and re.search(r"[가-힣0-9]", r["reply"].get("text") or ""):
            판단, c, 까닭 = 짧은댓글판단(r)
            # 깃허브(공개) 실행 화면엔 손님 글을 안 찍는다
            runlog("  짧은 댓글 @%s %s → AI %s(%d) %s" % (user, repr(text[:20]) if WHO == "PC" else "", 판단, c,
                                                        까닭[:60] if WHO == "PC" else ""))
            if 판단 == "이어짐" and c >= 짧은판단확신:
                why = None
            else:
                why = "%s · AI 도 끝난 말로 봄(%s)" % (why, 까닭[:60])
        if why:
