...
TURN_GUIDE = {
    ...
    # 2026-09-24: "물어라"라고만 적어 놨는데 검사는 **물음표로 끝나야** 통과였다.
    # 시키는 말과 검사 기준이 엇갈려서, 5턴 넘어 톤이 "짧고 따뜻하게"로 바뀌면
    # AI 가 따뜻한 문장으로 닫고 세 번 다 퇴짜를 맞아 멀쩡한 풀이가 통째로 버려졌다
    # (@coco_eunstar 7턴 '없지ㅋ'). 그래서 끝맺는 법을 말로도 못 박는다.
    ...
    # 2026-09-23 사장님 지시: "스하리 달라"고 청하지 말고 **리포스트 해주면 고맙겠다**는 뉘앙스로.
    # 받아내는 말이 아니라 고마워하는 말이라 부담이 적고, 리포스트는 글이 퍼져서 우리한테도 제일 크다.
    "마무리": ("이제 어느 정도 봐준 셈이다. 마무리하면서 **리포스트를 청하되, 달라고 하지 말고 '해주면 고맙겠다'는 결로** 써라.\n"
               "   이번 글의 결: **{promo}**\n"
               "   이 뜻은 살리되 문장은 네 말로 다시 써라. 그대로 베끼지 마라.\n"
               "   그리고 더 자세한 건 프로필에서 볼 수 있다고 한 마디. **링크 주소는 절대 쓰지 말고 '프로필'이라고만** 한다.\n"
               "   구걸하듯 매달리지 말고 툭 던지듯 짧게 한 줄.\n"
               "   '스하리'라는 말은 쓰지 않는다 — 품앗이 화폐 느낌이라 우리 결이 아니다.\n"
               "   ★ 줄 순서를 반드시 이렇게 한다: … 풀이 … → (끝에서 둘째 줄) 리포스트·프로필 한 줄\n"
               "     → **(맨 끝 줄) 물음표로 끝나는 질문 한 줄.**\n"
               "   리포스트 줄로 글을 끝내면 안 된다. 그 뒤에 질문이 한 줄 더 와야 한다.\n"
               "   여기서 대화가 끊기면 채널이 안 큰다."),
}
...
def write_reply(row, facts, missing, turn, fix=""):
    ...
    if missing:
        ...
        guide = TURN_GUIDE["마무리"].format(promo=random.choice(PROMO_LINES))
    else:
        guide = TURN_GUIDE["보통"]
    if row.get("같은날"):
