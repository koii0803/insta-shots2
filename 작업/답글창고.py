...
class 창고:
    ...
    def 푼사람_읽기(self):
        """{손님이름: 마지막으로 풀어 준 날짜}. 같은 사람이 며칠 만에 또 왔는지 본다.

        2026-09-25 사장님 지시: **7일 안에 이미 봐준 사람은 조용히 무시한다.**
        한 글 안에서는 상한 5로 막히는데, 손님이 **다른 글**에 또 달면 새로 5턴을
        통째로 받아 갔다. 지금은 안 샜지만(87명 중 0명) 막는 장치가 없었다.
        이름만 적는다 — 생년월일 같은 건 안 넣으니 지울 것도 없다."""
        return self.읽기("푼사람.json") or {}

    def 푼사람_적기(self, 이름, 날짜):
        cur = self.푼사람_읽기()
        cur[이름] = 날짜
        self.쓰기("푼사람.json", cur)
