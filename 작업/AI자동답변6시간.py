...
한바퀴상한 = 5         # 한 번에 이만큼만. 짧은 댓글이 몰려도 우르르 안 나가게
...
**이어짐** — 대화를 이어 가야 한다
  ...
  · 앞 풀이에 대한 반응이라 받아 줄 말이 있다 (예: "맞아" "아닌데")
...
# ── 한 바퀴 ────────────────────────────────────────────────────────
def 짧은것만(tok, uid, 본것):
    """본체가 '짧다'고 버릴 것만 골라 온다. 긴 것은 본체 몫이라 손 안 댄다."""
...
def 한바퀴(dry=False, 보기만=False):
    ...
    줄 = 짧은것만(tok, uid, 본것)
    if not 줄:
        봇.runlog("AI자동답변: 걸린 것 없음")
        return 0

    봇.runlog("AI자동답변: 짧아서 버려진 것 %d건" % len(줄))
    if 보기만:
        for r in 줄:
