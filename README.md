...
- `쇼츠예약.json` — 예약표. 한 건 = id, 영상 주소(R2), 캡션, 예약 시각, 상태(대기·게시·실패), 유튜브 주소. 액션이 `ig_container_id`·`ig_media_id`·`posted_at`·`attempts`·`last_error`를 덧붙인다.
  유튜브(2026-09-19): PC 가 `youtube_status: 대기`·`yt_title`·`yt_description`·`yt_tags` 를 실으면 액션이 R2 에서 받아 유튜브에 예약 공개로 올리고 `youtube_url`·`youtube_video_id`·`youtube_status`(예약/공개/실패) 를 적는다. 토큰은 금고의 `youtube_*`(PC `youtube_token.py`).
